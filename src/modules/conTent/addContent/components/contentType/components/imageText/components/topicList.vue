<template>
	<div>
					...
					<template #titleName="{text,record}">
						<a-tooltip>
							...
							<a-button type="link" @click="lookContentDetail(record.contentId)">
								{{ record.titleName.length > 15 ? record.titleName.slice(0,15) + '...' : record.titleName}}
							</a-button>
          	</a-tooltip>
					</template>
				</a-table>
      ...
      <a-col :xs="16" :sm="16" :xl="16" :xxl="16"></a-col>
		...
		>
			...
			<a-row>
				<a-col :push="4" :span="20">
					<p style="font-size: 14px;font-weight: 400;color: #666666;margin-bottom: 32px;">确定添加话题吗？?</p>
				</a-col>
			</a-row>
			<a-row>
				<a-col :push="4" :span="20" style="margin-bottom: 16px;display: flex;justify-content: end;align-items: center;">
					...
					<a-button type="primary" danger style="width: 84px;background: #E8380D;border-radius: 4px;" @click="makeSureAddTopic">确定</a-button>
				</a-col>
			</a-row>
		</a-modal>
		...
		>
			<template #title>
				<div class="ScreeningCondition_topic_title">
					<div>
...
<style lang="less" scoped>
	...
	.ScreeningCondition_topic_title {
  display: flex;
