...
<template>
  <div>
		<a-row>
			<a-col :span="3" style="line-height: 32px;">所属渠道：</a-col>
			<a-col :span="20">
		...
		<a-row style="margin-top:2%">
			<a-col :span="3" style="line-height: 32px;">运营位名称：</a-col>
			<a-col :span="15">
				<a-input
					...
					maxlength="60"
					:showCount="true"
					placeholder="请输入运营位名称，支持中英文、数字、符号"
					style="width:594px;height:32px;border-radius: 4px;"
				></a-input>
			</a-col>
		</a-row>
		<a-row style="margin-top:2%">
			<a-col :span="3" style="line-height: 32px;">运营位封面：</a-col>
			<a-col :span="5">
...
export default class addInformation extends Vue {
	...
	public async getContentList(){
      ...
      request: (header: AxiosRequestHeaders) => {
		...
		await operaTionApi.getContentList(params,header).then((res:any) => {
			if(res.flag === 'success'){
				store.commit('changeContentAllNumber',res.recordsTotal)
	...
	public editTest:any = computed(() =>{
		...
		if(this.editTypeValue === '1'){
			...
			this.coverLength = chooseEditInfo.reachPictureLong
			this.coverWidth = chooseEditInfo.reachPictureWide
