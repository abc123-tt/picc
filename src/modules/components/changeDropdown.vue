<template>
    <div class="">
        <a-dropdown placement="bottom" :trigger="['hover']" @click.stop>
            <div :style="{
                textAlign: 'center',
                color: '#EF6A49',
            }">
                <div class="choosedown" v-if='showLeftFolder'>
                    <img src="@/modules/assets/内容管理-列表_slices/icon／操作.png" alt="">
                </div>
                <div class="choosedown" v-if='showTableActive'>
                    <img src="@/modules/assets/内容管理-列表_slices/icon／操作1.png" alt="">
                </div>
            </div>
            <template #overlay>
                <a-menu>
                    <a-menu-item @click="chooseDropdown(item)" v-for="item in menuList" :key="item"
                        :disabled="item.disabled">
                        <div class="dropdown">
                            <component :is="item.iconName" style="margin-right: 10px" v-if="item.iconName" />
                            {{ item.title }}
